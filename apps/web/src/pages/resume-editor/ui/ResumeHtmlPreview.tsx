...
export const ResumeHtmlPreview = memo(function ResumeHtmlPreview({ avatar, resume }: ResumeHtmlPreviewProps) {
        ...
        {profile.fullName || profile.targetRole || contacts.length || profile.links.length || avatar ? (
          ...
          >
            <div className={cn('min-w-0 flex-1', isClassic ? 'text-center' : 'text-left')}>
              ...
              {profile.targetRole ? <p className="mt-[0.45em] text-[1.18em] text-[#5d5962]">{profile.targetRole}</p> : null}
              {contacts.length || printableLinks.length ? (
                <div className={cn('mt-[0.8em] flex flex-wrap gap-x-[1em] gap-y-[0.35em] text-[0.82em] text-[#77717a]', isClassic ? 'justify-center' : 'justify-start')}>
                  {contacts.map((value) => <span key={value}>{value}</span>)}
                  {printableLinks.map((link) => (
                    <Link className="text-inherit no-underline hover:underline" href={link.url} key={link.id} rel="noreferrer" target="_blank">{link.label}</Link>
                  ))}
                </div>
              ) : null}
            </div>
            {avatar ? <img alt="" className={cn('ml-[2em] size-[7.2em] shrink-0 object-cover', isClassic ? 'rounded-full' : 'rounded-md')} src={avatar} /> : null}
          </header>
        ) : null}

        {resume.content.sections.filter(sectionHasPrintableContent).map((section) => (
          <HtmlSection isClassic={isClassic} key={section.id} section={section} sectionGap={formatting.sectionGap} />
        ))}
      </article>
    </Card>
  );
...
function HtmlSection({ isClassic, section, sectionGap }: { isClassic: boolean; section: ResumeSection; sectionGap: keyof typeof sectionGapClasses }) {
  return (
    <section className={cn('break-inside-avoid', sectionGapClasses[sectionGap])}>
      <h2
        className={cn('border-b pb-[0.35em] text-[1.18em] font-bold tracking-[0.04em]', isClassic ? 'text-[#27242a]' : 'text-[var(--resume-accent)]')}
        style={{ borderColor: isClassic ? '#b9b4bc' : 'var(--resume-accent)' }}
      >
        {section.title}
      </h2>
      {section.type === 'summary' ? <p className="mt-[0.8em] whitespace-pre-line">{section.text}</p> : null}
      {section.type === 'skills' ? (
        <div className="mt-[0.8em] flex flex-wrap gap-[0.65em]">
          {section.items.filter((item) => item.name.trim()).map((item) => (
            <span
              className={cn('inline-flex items-center gap-[0.4em] border px-[0.8em] py-[0.35em]', isClassic ? 'rounded-sm border-[#d6d2d5] bg-[#f3f1f2] text-[#38343a]' : 'rounded-full bg-[color-mix(in_srgb,var(--resume-accent)_7%,white)] text-[var(--resume-accent)]')}
              key={item.id}
              style={isClassic ? undefined : { borderColor: 'color-mix(in srgb, var(--resume-accent) 28%, white)' }}
            >
              <span>{item.name}</span>
              {item.level ? <span className="opacity-75">· {SKILL_LEVEL_LABELS[item.level]}</span> : null}
            </span>
          ))}
        </div>
      ) : null}
      {section.type !== 'summary' && section.type !== 'skills' ? (
        <div className="space-y-[1em] pt-[0.8em]">
          {section.items.filter(itemHasPrintableContent).map((item) => {
            const display = getEntryDisplay(section.type, item as unknown as Record<string, unknown>);
              ...
              <article className="break-inside-avoid" key={item.id}>
                {display.title || display.date ? (
                  <div className="flex items-baseline justify-between gap-[1.5em]">
                    ...
                    {display.date ? <time className="shrink-0 text-[0.82em] text-[#6f6972]">{display.date}</time> : null}
                  </div>
                ) : null}
                {display.subtitle ? <p className="mt-[0.15em] text-[0.82em] text-[#6f6972]">{display.subtitle}</p> : null}
                {bullets.length ? (
                  <ul className="mt-[0.45em] space-y-[0.18em]">
                    {bullets.map((line, index) => (
                      <li className="flex gap-[0.65em]" key={`${index}-${line}`}>
                        <span aria-hidden="true" className="text-[var(--resume-accent)]">•</span>
                        <span>{line}</span>
                      </li>
                    ))}
                  </ul>
