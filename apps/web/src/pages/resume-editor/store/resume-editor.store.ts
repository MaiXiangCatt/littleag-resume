...
export const useResumeEditorStore = create<EditorState>()((set, get) => ({
  ...initialState,
  load: (document) => set({ document, error: null, isLoading: false, saveStatus: 'saved', changeVersion: 0 }),
  loadFailed: (message) => set({ error: message, isLoading: false }),
  setLoading: () => set({ error: null, isLoading: true }),
  updateDraft: (updater) => {
    const current = get().document;
    if (!current) return;
    set((state) => ({ document: updater(current), changeVersion: state.changeVersion + 1, saveStatus: 'dirty', error: null }));
  },
  setSaving: () => set({ saveStatus: 'saving', error: null }),
  applySaved: (document, savedVersion) => set((state) => ({
    document: state.document ? { ...state.document, revision: document.revision, exportCount: document.exportCount, updatedAt: document.updatedAt, hasAvatar: document.hasAvatar } : document,
    saveStatus: state.changeVersion === savedVersion ? 'saved' : 'dirty',
    error: null,
  })),
  mergeServerMetadata: (document) => set((state) => ({
  ...
  setConflict: () => set({ saveStatus: 'conflict', error: '这份简历已在其他页面更新，请选择保留哪个版本。' }),
  replaceDocument: (document) => set({ document, error: null, isLoading: false, saveStatus: 'saved', changeVersion: 0 }),
