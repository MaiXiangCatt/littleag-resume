...
> Workflow：`full`
> 初始化来源：`.vega-harness/docs/prd-home-page.md`
> 原始 PRD 来源：`docs/designAndPrd/home_page_prd_v3.md`

## 本轮用户补充输入

- 本需求从原首页 Lite 范围扩大为 Full 范围：前端 Home Page、AuthModal、后端登录/注册 API、OpenAPI YAML 契约，以及前后端接口生成同步都纳入本次需求。
- Hero 右侧预览区先使用占位预览卡片。
- 「查看示例」按钮目标为首页内占位示例预览。
- 移动端和 PC 端登录/注册弹窗统一使用 Dialog。
