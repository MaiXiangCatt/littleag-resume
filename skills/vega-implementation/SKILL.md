...
读取 `documents.openspec_dir` 指向的 OpenSpec change，按 `tasks.md` 逐项实现。Lite workflow 以一个活跃需求的主 change 为实施单元；Full workflow 必须读取 `vega module list --json`，优先处理 pending 模块，并在模块完成后调用 `vega module complete <module-name>`。
...
- `vega verify --json` 失败：先报告并修复状态完整性问题，不开始实现；
...
将 pending 模块作为本次实现队列。若模块列表为空，说明 breakdown 阶段没有登记模块，停止并要求补齐模块；若全部模块已完成，但 `tasks.md` 仍有未完成项，先核对 OpenSpec tasks 和模块状态是否不一致，不要直接推进。
...
- 当前处理的模块；Lite workflow 可记为 `<requirement>`；
- 任务对应的 spec scenario；
- 预期改动文件；
- 应运行的测试命令；
...
6. **勾选任务**
   ...
   - Full workflow 中，同步检查当前模块是否已无未完成任务；满足时调用 `vega module complete <module-name>`；
   - 不要批量勾选未验证任务。
...
所有模块必须为 `completed`，否则不要推进 `implementation` 阶段。
...
- Full workflow 中所有模块已通过 `vega module complete` 标记完成；
- 相关 Makefile 验证命令通过；
- 未回滚用户无关改动，未自动提交；
