...
`vega-tech-design` 只服务 Full workflow。它把 brainstorm 设计转成可执行、可验收的技术方案，为后续 `vega-breakdown` 和 `vega-openspec` 提供稳定输入。
...
- 不把缺少证据的接口、数据结构、架构取舍写成事实；不确定内容必须标为假设、风险或开放项。
- 不在技术设计未自检通过前执行 `vega complete`。
- 不把用户批准作为生成 TD 或推进阶段的默认前置条件；用户验收发生在产物生成和阶段推进之后。

## 默认自主推进

- 默认基于 PRD、brainstorm、代码结构和仓库约束生成 TD，自检通过后登记并完成阶段。
- 只有技术取舍会造成不可逆成本、外部依赖采购、数据破坏、安全风险，或上游需求互相冲突导致无法成文时，才暂停询问用户。
- 所有非阻塞不确定性写入“风险、依赖与开放项”和最终摘要，供用户产后验收。
...
为以下步骤建立任务清单并逐步更新。不要把非阻塞验收点当作暂停关卡。
...
先给推荐方案和理由。默认采用推荐方案写入 TD；若方案选择会显著影响接口、数据结构、模块边界或长期维护成本，在 TD 中记录取舍依据和验收关注点。只有该选择不可逆或风险高到无法安全假设时，才先向用户确认。
...
在 TD 中补充“用户验收关注点”或等价小节，列出需要用户重点检查的接口、数据、模块边界、风险和开放项。用户要求修改时，后续可回到 `tech_design` 修订文档并重新执行质量门禁。不要等待用户明确批准才进入阶段完成。
...
- 关键技术取舍无法安全假设时，保持 `tech_design` 为 `in_progress`，向用户提出最小必要问题。
