...
- 用户说“继续”“推进当前需求”时，默认恢复上下文并执行 `vega next --json` 返回的阶段 Skill；如果阶段 Skill 自检通过并推进了阶段，可继续按最新路由推进，直到遇到真实阻塞、完成需求或用户要求停下。
- 不把阶段产物的用户验收作为 `continue` 的默认暂停点；验收关注点由具体阶段 Skill 写入产物和摘要，用户可事后要求修订。
- 只有状态损坏、关键产物缺失且无法恢复、failed 阶段、破坏性/不可逆操作、或具体阶段 Skill 明确报告阻塞时，才停止等待用户输入。
...
3. 如果已经越过产物生产阶段，优先尝试从确定性路径恢复并补登记；无法唯一确定时才询问用户是否补登记路径，或返回前置 Skill 修正；
...
如果被路由 Skill 因真实阻塞暂停，不要额外推进阶段；只报告暂停点和下一步需要用户提供的内容。非阻塞产后验收点不应阻止继续路由。

## 常见场景

### 用户说“继续”

执行上下文重建，读取 `vega next --json`，直接进入返回的 Skill；阶段完成后可继续按最新路由推进，直到完成、阻塞或用户要求停下。
