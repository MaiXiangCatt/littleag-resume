...
- 默认执行验证、修复可安全修复的问题、生成验证报告、登记报告并推进到 archive。
- 不把 Medium/Low 风险或非本次范围的工具链缺口作为等待用户批准的默认关卡；记录到报告和最终摘要即可。
- 只有 Critical/High 问题无法修复、关键验证命令无法运行且影响结论、或需要扩大需求范围时，才暂停询问用户或标记失败。
...
- 所有必需命令通过，或非本次范围的工具链缺口已明确记录且不影响本次结论；
...
- 发现阻塞问题且无法继续时执行：
  ...
  ```
- Critical/High 问题、阻塞级工具链缺口或需求范围外修复需要用户决策时，不推进阶段。
