...
- 默认从用户输入中推断需求名和 workflow，直接初始化并推进到 brainstorm。
- 只有需求名冲突、完全缺少需求输入、文档读取失败且没有可用描述、或 workflow 选择会明显改变后续流程成本时，才暂停询问用户。
- 推断出的名称、workflow 和依据必须写入最终摘要，供用户产后验收；用户不满意时可切换/新建需求或回到 init 修订。

## 执行流程

严格按顺序执行。能从仓库和输入可靠推断的内容不要重复询问；必须提问时每条消息只问一个问题。
...
3. 暂时无法读取文档时，若用户消息中已有足够需求描述，则基于现有描述继续并在 PRD 快照中记录文档缺失；若没有可用描述，再让用户补充文档或需求摘要。
...
用户已明确给出合法名称时直接采用；否则使用推荐名称继续，不等待用户确认。
...
- 如果从上下文能明确判断用户要继续旧需求：执行 `vega requirement switch <name>`，读取状态并按当前阶段路由；
- 如果用户明确要创建新需求或上下文无法判断：生成带短后缀的新名称并继续；
- 禁止删除或覆盖已有状态文件。
...
根据需求推荐一种并说明一句理由，然后直接采用。无法判断时默认采用 `lite`，并在 PRD 快照和最终摘要中说明该假设。
